 ...
 *
 * $Id: ReFService.java 1142 2007-07-18 20:49:58Z gnovelli $
 *
 */
...
public class ReFService extends PlatformService {
    ...
    private void updateGISCache() {
        ...
        } else if ((super.clock() - this.getGisRepositoryCache()
        .getLastRequestTime()) > (this.getGISCacheLifetime())) {
            gisReply = this.searchGIS();
            this.setGisRepositoryCache(gisReply.getGIS());
        }
    }
    ...
    private void updateNMCache() {
        ...
        } else if ((super.clock() - this.getNetworkMapCache()
        .getLastRequestTime()) > (this.getNMCacheLifetime())) {
            nmReply = this.queryNM();
            this.setNetworkMapCache(nmReply.getNetworkMap());
        }
    }
    ...
    private void sendPlayEndReply(ReFPlayReply playReply) {
        ...
        double evsend_time = 0;
        super.send(super.output, GridSimTags.SCHEDULE_NOW,
                        QAGESATags.REF_PLAY_END_REP, new IO_data(playReply, SIZE, playReply.getRequest()
                                        .getUserID()));
        evsend_time = GridSim.clock();
    }
    ...
    protected MuMSearchReply requestGEList(String movieTag) {
        ...
        if (reply.isOk()) {
            msg = String.format(
                    ...
                    evrecv_time, reqrepID, this.get_name(), movieTag, reply
                    .getGelist());
        } else {
            msg = String.format(
                    "%1$f %2$d %3$s <-- MuM MUM_SEARCH_REPLY %4$s %5$s",
    ...
    protected GISReply searchGIS() {
        ...
        int reqrepID = request.getReqrepID();
        super.send(super.output, GridSimTags.SCHEDULE_NOW,
                QAGESATags.GIS_REQUEST, new IO_data(request, SIZE, super
                .getEntityId("GISService")));
        evsend_time = GridSim.clock();
        ...
        Assert.assertEquals(requestID, reply.getRequestID());
        Assert.assertEquals(QAGESATags.GIS_REQUEST, reply
                .getRequestTAG());
    ...
    protected NetworkMapReply queryNM() {
        int SIZE = 500;
        double evsend_time = 0;
        NetworkMapRequest request = new NetworkMapRequest(this.get_id(), this
                .get_id());
        int requestID = request.getRequestID();
        int reqrepID = request.getReqrepID();
        super.send(super.output, GridSimTags.SCHEDULE_NOW,
                Tags.NM_NETWORKMAP_REQ, new IO_data(request, SIZE, this
                .getAgentPlatform().get_id()));
        evsend_time = GridSim.clock();
