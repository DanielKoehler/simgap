...
public abstract class VirtualOrganization extends AbstractVirtualOrganization {
	...
	public VirtualOrganization(boolean traceFlag, int numCE, int MIPS,
			int PEMax, int MMin, int MMax, int numSE, int GBMin, int GBMax,
			int routersPerCloud, int clouds, boolean fixedInfrastructure,
			double factor, int numUsers, int experimentID) throws Exception {
		this.setFactor(factor);
		this.initParameters(traceFlag, numCE, MIPS, PEMax, MMin, MMax, numSE,
				GBMin, GBMax, routersPerCloud, clouds, fixedInfrastructure,
				factor, numUsers, experimentID);
	...
	public VirtualOrganization(boolean traceFlag, int numCE, int MIPS,
			int PEMax, int MMin, int MMax, int numSE, int GBMin, int GBMax,
			int numUsers, double factor) throws Exception {
		this.setFactor(factor);
		this.initParameters(traceFlag, numCE, MIPS, PEMax, MMin, MMax, numSE,
				GBMin, GBMax, numUsers);
	...
	protected void initializeCEs() {
		...
		for (int i = 0; i < this.getNumCEs(); i++) {
			GridElement computingElement = (GridElement) Sim_system
					.get_entity("CE_" + i);
			this.mapCEs.put(computingElement.get_id(), computingElement
					.getExternalRouter());
	...
	protected void initializeSEs() {
		for (int i = 0; i < this.getNumSEs(); i++) {
			GridElement storageElement = (GridElement) Sim_system
					.get_entity("SE_" + i);
			this.mapSEs.put(storageElement.get_id(), storageElement
					.getExternalRouter());
	...
	protected void createEntities() throws Exception {
		...
		if (this.getNetworkType() == VirtualOrganization.NT_RINGSCHAIN) {
			this.setTopology(new RingsChain(this.getRoutersPerCloud(), this
					.getClouds(), this.getFactor(), this.isTraceFlag()));
		} else if (this.getNetworkType() == VirtualOrganization.NT_STATIC) {
			this.setTopology(new StaticTopology(this.isTraceFlag(), this
					.getFactor()));
		}
		...
		COREGEFactory sefactory = new COREGEFactory(this.getTopRegionalRC(),
				this.getBasicMIPS(), getPEMax(), getMMin(), getMMax(),
				getGBMin(), getGBMax());
	...
	public void createAndAttachCEs() throws Exception {
		if (this.getNetworkType() == VirtualOrganization.NT_STATIC) {
			...
			for (int i = 0; i < this.getNumCEs(); i++) {
				index = i % N;
				RIPRouter router = (RIPRouter) Sim_system.get_entity("ROUTER_"
						+ index);
				Link link = LinkFactory.GELink(false);
				GridElement computingElement = this.seFactory.create(this
						.isFixedInfrastructure(), i, link, false);
		...
		if (this.getNetworkType() == VirtualOrganization.NT_STATIC) {
			...
			for (int i = 0; i < this.getNumSEs(); i++) {
				...
				RIPRouter router = (RIPRouter) Sim_system.get_entity("ROUTER_"
						+ index);
				Link link = LinkFactory.GELink(false);
				GridElement storageElement = this.seFactory.create(this
						.isFixedInfrastructure(), i, link, true);
