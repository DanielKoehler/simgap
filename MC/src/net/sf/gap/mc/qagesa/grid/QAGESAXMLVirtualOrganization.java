 ...
 *
 * Created on 4 February 2008, 18.40 by Giovanni Novelli
 *
 * $Id$
 *
 */
...
public class QAGESAXMLVirtualOrganization extends AbstractVirtualOrganization {
    ...
    public QAGESAXMLVirtualOrganization(
            String xml,
            String xsd,
            double factor, 
            int numUsers, 
        ...
        for (int i = 0; i < numGEs; i++) {
            GridElementType ge =
                    scenario.getGrid().getGridElements().get(i);
            if (ge.isSE()) {
                countSEs++;
            } else {
                countCEs++;
    ...
    public void createAndAttachCEs() throws Exception {
        for (int i = 0; i < this.getNumCEs()+this.getNumSEs(); i++) {
            ...
            if (getScenario().getTopology().getMapLinks().
                ...
                LinkType linkItem =
                        getScenario().getTopology().getMapLinks().
                        get(linkName);
                if (linkItem.isBidirectional()) {
                    ...
                    double cost = 1.0;
                    MachineListType mListType =
                            geItem.getMachineList();
                    ...
                    try {
                        ...
                        if (geItem.isSE()) {
                            ...
                            for (int ihd = 0;
                                    ihd <
                                    geItem.getStorage().getHardDiskList().
                                    getItems().
                                    size(); ihd++) {
                                HardDiskType hdType =
                                        geItem.getStorage().
                                        ...
                                        getItems().get(ihd);
                                Storage storage =
                                        new HarddriveStorage(hdType.getName(),
                            ...
                            for (int it = 0; 
                                ...
                                TapeType tapeType = 
                                        ...
                                        getItems().get(it);
                                Storage storage = 
                                        new TapeStorage(tapeType.getName(),
                        ...
                        ge.createLocalRC();
                        ge.setHigherReplicaCatalogue(
                                TopRegionalRC.DEFAULT_NAME);
                        ge.setNumPE(np);
                        ge.setNumWN(m);
                    } catch (Exception e) {
                        e.printStackTrace();
