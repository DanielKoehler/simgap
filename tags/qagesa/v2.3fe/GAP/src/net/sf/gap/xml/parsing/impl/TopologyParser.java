...
public class TopologyParser extends Parser {
	...
	public NetworkTopologyType getTopology() {
		...
		NodeList routerItems = this.getDocument().getElementsByTagName(
				"routerItem");
		...
		NodeList netLinkItems = this.getDocument().getElementsByTagName(
				"netLinkItem");
		for (int i = 0; i < netLinkItems.getLength(); i++) {
			...
			Element element;
			element = (Element) linkItem.getElementsByTagName("Baudrate").item(
					0);
			...
			element = (Element) linkItem.getElementsByTagName("fromEntity")
					.item(0);
			String fromEntity = element.getTextContent();
			element = (Element) linkItem.getElementsByTagName("toEntity").item(
					0);
			String toEntity = element.getTextContent();
			element = (Element) linkItem.getElementsByTagName("bidirectional")
					.item(0);
			boolean aBidirectional = true;
			if (element != null) {
				aBidirectional = Boolean.parseBoolean(element.getTextContent());
			}
			LinkType link = new LinkType(aName, aBaudrate, aDelay, aMTU,
					fromEntity, toEntity, aBidirectional);
			topology.addLink(link);
		}
		NodeList geLinkItems = this.getDocument().getElementsByTagName(
				"geLinkItem");
